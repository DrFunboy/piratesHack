...
$date_pattern = 'eee=dd=MM'; // Будет разбиваться знаком равно и подставляться в $fmt_pattern
$fmt_pattern = '<b>%s</b>, %s.%s';

$formatter = new IntlDateFormatter($modx->getOption('locale'), IntlDateFormatter::FULL, IntlDateFormatter::FULL);
$formatter->setPattern($date_pattern);
...
for ($i = 1; $i <= 7; $i++) {
    $json['dates'][] = array(
        ...
        'datefmt' => vsprintf($fmt_pattern, explode('=', $formatter->format($date))),
    );
...
if (empty($_REQUEST['datestart'])) {
    ...
    $wrepeat[] = "weekday(sch.datestart) = weekday('{$json['datestart']}')"; // Add to array
}
...
$wSchedule->select(array(
    "sch.*",
    //'IFNULL(schParent.id, 0) as parentid',
...
$wSchedule->innerJoin('idStatus');
/*$wSchedule->innerJoin('idStatus', 'idStatus', array(
    'idStatus.alias = sch.stype',
    "idStatus.tbl = 'idSchedule'",
));*/

//$wSchedule->leftJoin('idSchedule', 'schParent', array('schParent.id = sch.parent'));
$wSchedule->leftJoin('idTrainer', 'tr2', array('tr2.id = sch.trainer2'));
$wSchedule->leftJoin('idSquad');
$wSchedule->leftJoin('idClub', 'sq_club', array('sq_club.id = idSquad.club'));
$wSchedule->leftJoin('idLevel', 'idLevel', array('idLevel.id = idSquad.lvl'));
...
# Если в этот период у находится родительский элемент, то он исключается, остается только наследник
